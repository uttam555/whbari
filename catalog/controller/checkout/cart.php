...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
		...
		$category_id = isset($_GET['checkout'])?(int)$this->request->get['checkout']:0;
		$this->data['category_info'] = $this->model_catalog_category->getCategory($category_id);
		...
		if (!empty($this->request->post['quantity'])) {
			...
			if($category_id){
				$this->redirect($this->url->link('checkout/cart','checkout='.$category_id,'SSL'));
			}else{
		...
		// Remove Category Cart
		if (isset($this->request->get['remove_category'])) {
			$this->cart->remove_category($this->request->get['remove_category']);
			
			unset($this->session->data['vouchers'][$this->request->get['remove_category']]);
			...
			if($category_id){
				$this->redirect($this->url->link('checkout/cart','checkout='.$this->request->get['remove_category'],'SSL'));
			}else{
				$this->redirect($this->url->link('checkout/cart'));  			
			}
		}		
...
//$this->data['action'] = $this->url->link('tool/export', 'token=' . $this->session->data['token'], 'SSL');
			...
			if($category_id){
				$this->data['action'] = $this->url->link('checkout/cart','checkout='.$category_id);   
			}else{
...
//echo "<pre>";print_r($products);echo "</pre>";
            foreach ($products as $product) {
                ...
                $this->data['products'][] = array(
                    ...
                    'remove'              => $this->url->link('checkout/cart', 'remove=' . $product['key']."&checkout=".$category_id),
			...
			if (!empty($this->session->data['vouchers'])) {
				foreach ($this->session->data['vouchers'] as $key => $voucher) {
					$this->data['vouchers'][] = array(
						...
						'remove'      => $this->url->link('checkout/cart', 'remove=' . $key."&catalog=".$category_id)   
					);
				}
			}
...
$this->load->model('total/' . 'cart_sub_total');							
						$this->{'model_total_' . 'cart_sub_total'}->getTotal($total_data, $total, $taxes);
						}else{
$this->load->model('total/' . $result['code']);							
$this->{'model_total_' . $result['code']}->getTotal($total_data, $total, $taxes);							
			...
			if($category_id){
				$this->data['checkout'] = $this->url->link('checkout/checkout', 'checkout='.$category_id, 'SSL'); 
			}else{
	...
	public function bulk_add(){
		$category_id = isset($_GET['checkout'])?$this->request->get['checkout']:0;
		
		$carcats = $this->cart->_getSubCategories();
		
		if(in_array($category_id,$carcats)){
			$cart_link = $this->url->link('checkout/cart', 'checkout='.$category_id);	
		}else{
		...
		foreach ($this->cart->getAllProducts() as $product) {
			$cart_product[] = $product['product_id'];
			$cart_option[$i]['product_id'] = $product['product_id'];
			foreach($product['option'] as $option){
				if($option['option_id']=='11') $cart_option[$i]['s'] = $option['product_option_value_id'];
				elseif($option['option_id']=='13') $cart_option[$i]['c'] = $option['product_option_value_id'];
			}
			$cart_option[$i]['quantity'] = $product['quantity'];
			$i++;
		}	
		$json = array();
		...
		if ($product_info) {			
			...
			if (!$json) {
				...
				if($items[0]=="q" && count($items)==3 && ((int)$this->request->post[$key]>0 || $this->request->post[$key]=='0')){
	
					$option_size = $this->db->query("SELECT * FROM `".DB_PREFIX."product_option_value` WHERE product_option_value_id='".$items[1]."'");
					$option_color = $this->db->query("SELECT * FROM `".DB_PREFIX."product_option_value` WHERE product_option_value_id='".$items[2]."'");
					
					$option[$option_size->row['product_option_id']] = $items[1];
					$option[$option_color->row['product_option_id']] = $items[2];
					$cnt++;
						if($this->_check_cart_data($cart_option,$this->request->post['product_id'],$option_size->row['product_option_value_id'],$option_color->row['product_option_value_id'])<=0){
							$this->cart->add($this->request->post['product_id'], $this->request->post[$key], $option, $profile_id);
							
							$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], $cart_link);
						}else{
					//update
						$ky = (int) $this->request->post['product_id'] . ':';
						if ($option) {
							$ky .= base64_encode(serialize($option)) . ':';
						}  else {
							$ky .= ':';
						}
	
						$this->cart->update($ky,$this->request->post[$key]);
						$json['success'] = sprintf($this->language->get('text_success_update'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], $cart_link);
				...
				if($cnt == 0){
					$json['error']['bulk'] = "No item is added";
				}else{
					//$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], $this->url->link('checkout/cart'));
				}
				unset($this->session->data['shipping_method']);
	...
	private function _check_cart_data($cart_option,$product_id,$size,$color){
		foreach($cart_option as $option){
			if($option['product_id'] == $product_id && $option['s']==$size && $option['c']==$color){
